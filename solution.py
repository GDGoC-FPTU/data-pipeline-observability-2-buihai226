...
def validate(data):
    ...
    for record in data:
        ...
        # Check if price is positive and category is not empty
        if price > 0 and category:
            valid_records.append(record)
        else:
            error_count += 1
            if price <= 0:
                print(f"  ⚠️  Dropped record {record.get('id')}: Invalid price ({price})")
            if not category:
                print(f"  ⚠️  Dropped record {record.get('id')}: Empty category")
